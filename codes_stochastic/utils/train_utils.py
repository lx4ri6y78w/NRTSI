...
def run_epoch(epoch, total_epoch, train, model, exp_data, clip, max_gap, min_gap, max_level, confidence, optimizer=None, 
            batch_size=64, n_mix=1, teacher_forcing=True, fig_path=None, cur_lr=1e-4, save_all_imgs=False, dataset='billiard', gp=0):
    if train:
        model.train()
    else:
        model.eval()
                ...
                if min_gap < gap and gap <= max_gap: 
                    ...
                    if confidence:
                        if n_mix == 1:
                            ...
                            level_loss = nll_gmm(target_data, prediction, n_mix)
                        ...
                        if save_all_imgs:
                            for j in range(batch_size):
                                if dataset == 'billiard':
                                    ...
                                    plot_nfl(epoch, fig_path, obs_data, target_data.transpose(0,1), ground_truth, prediction, gap, n_mix, i, j)
                            ...
                            if dataset == 'billiard':
                                plot(epoch, fig_path, obs_data, target_data.transpose(0,1), ground_truth, prediction, gap)
                                    ...
                                    plot_nfl(epoch, fig_path, obs_data, target_data.transpose(0,1), ground_truth, prediction, gap, n_mix)
                            elif dataset == 'traffic' or dataset == 'mujoco':
...
def nll_gmm(gt, pred, n_mix, eps=1e-6):
...
def plot_nfl(epoch, fig_path, obs_data, target_data, ground_truth, prediction, gap, n_mix, i=0, j=0):
    target_data = target_data.cpu().numpy()
    ground_truth = ground_truth.cpu().numpy()
    obs_data = obs_data.detach().cpu().numpy()
    colormap = ['b', 'r', 'g', 'm', 'y']
    ...
    if n_mix > 1:
        eps = 1e-6
        mix = torch.distributions.Categorical(F.softplus(prediction[:,:,-n_mix:]))
        pred_mean = prediction[:, :, :ground_truth.shape[-1]*n_mix]
        pred_mean = pred_mean.view(prediction.shape[0], prediction.shape[1], n_mix, ground_truth.shape[-1])
        pred_std = F.softplus(prediction[:, :, n_mix*ground_truth.shape[-1]:2*n_mix*ground_truth.shape[-1]]) + eps
        pred_std = pred_std.view(prediction.shape[0], prediction.shape[1], n_mix, ground_truth.shape[-1])
        normal_distri = torch.distributions.Independent(torch.distributions.Normal(pred_mean,pred_std), 1)
        gmm = torch.distributions.MixtureSameFamily(mix, normal_distri)
        samples = gmm.sample().detach().cpu().numpy()
        for k in range(6):
            ax.scatter(target_data[:,j,k], target_data[:,j,k+6], color=colormap[0])
            ax.scatter(obs_data[j,:,k], obs_data[j,:,k+6], color=colormap[2])
            ax.scatter(samples[j,:,k], samples[j,:,k+6], color=colormap[1])
        for k in range(6):
            ax.plot(ground_truth[:,j, k], ground_truth[:,j,k+6], color=colormap[3])
    
    else:
        if prediction.shape[-1] == 24:
            prediction[:,:,12:] = F.softplus(prediction[:,:,12:])
        prediction = prediction.detach().cpu().numpy()
        
        for k in range(6):
            ax.scatter(target_data[:,j,k], target_data[:,j,k+6], color=colormap[0])
            ax.scatter(obs_data[j,:,k], obs_data[j,:,k+6], color=colormap[2])
            ax.scatter(prediction[j,:,k], prediction[j,:,k+6], color=colormap[1])
            ax.scatter(prediction[j,:,k], prediction[j,:,k+6], marker='o', c='none', s= 10**5 * prediction[j,:,k+12], edgecolor=colormap[1])
            ax.scatter(prediction[j,:,k], prediction[j,:,k+6], marker='o', c='none', s= 10**5 * prediction[j,:,k+18], edgecolor=colormap[1])
